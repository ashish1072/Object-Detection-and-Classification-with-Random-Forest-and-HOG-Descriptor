...
void HOGDescriptor::detectHOGDescriptor(cv::Mat &im, std::vector<float> &feats) {
    if (!is_init) {
        initDetector();
    }
  
    cv::Size pad = getPadSize();
    cv::Size win_size = getWinSize();
    std::cout<< ">> window size"<< win_size <<std::endl<<std::endl; 
   //std::cout<< "size of original image:" <<im.size()<<std::endl;

    if (im.rows < win_size.height || im.cols < win_size.width)
    {
        float scale = fmax((win_size.height * 1.0f) / im.rows, (win_size.width * 1.0f) / im.cols);
        cv::resize(im, im, cv::Size(0, 0), scale, scale);
    }
    
    //std::cout<< "size of resized image:" <<im.size()<<std::endl;

    cv::Rect region(15,0, win_size.width, win_size.height);
    im = im(region);
    std::cout<< ">> Resized image created"<<std::endl<<std::endl;

    cv::HOGDescriptor hog = getHog_detector();
    std::vector<float> descriptors;
    std::cout<< ">> Hog detector created"<<std::endl<<std::endl;

    cv::Mat gray_image;
    cv::cvtColor(im, gray_image, CV_RGB2GRAY );
    cv::Size block_step = getBlockStep();
    hog.compute(gray_image, descriptors, block_step, pad);
    visualizeHOG(im, descriptors, hog, 3);
...
void HOGDescriptor::visualizeHOG(cv::Mat img, std::vector<float> &feats, cv::HOGDescriptor hog_detector, int scale_factor) {
    ...
    std::cout<< ">> Visualization completed"<<std::endl<<std::endl;    
